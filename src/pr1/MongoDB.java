...
public class MongoDB {
    ...
    public static MongoClient mongo = new MongoClient("localhost", 27017); //connetct to mongo db server
...
//	 int num=0;
    ...
    public static void addUser(List<Status> st) throws IOException {
        ...
        query.put("User_Screen_name", user.getScreenName());
        try (DBCursor cursor = userDB.find(query)) {
            if (cursor.count() == 0) {
                String str = null;
    ...
    public static void requestByTopic(String str, Date date) {
        ...
        try (DBCursor cursor = tweetDB.find(q)) {
            ...
            while (cursor.hasNext()) {
                ...
                if (userID == ID) {
                    ...
                    if (sum != 0) {
                        ...
                        try (DBCursor uscursor = userDB.find(query)) {
    ...
    public static int[] specRequest(String topic, String subtopic, String group, Date date, String gender, Date from, Date to) {
        ...
        while (cursor.hasNext()) {
            ...
            try (DBCursor cursor1 = tweetDB.find(q)) {
                int sum = 0;
